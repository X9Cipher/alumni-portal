    ...
    io.on('connection', (socket) => {
      ...
      socket.on('send-message', async (data) => {
        try {
          ...
          const canMessage = await ConnectionService.canMessage(socket.data.userId, recipientId)
          if (!canMessage) {
            socket.emit('error', { message: 'Cannot send message. Users must be connected.' })
            return
          }
